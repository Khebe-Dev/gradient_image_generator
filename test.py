...
def parse_args():
    ...
    # Extract the points and output path
    point1 = sys.argv[1]
    point2 = sys.argv[2]
...
# Create BMP image with linear gradient
def create_file(x1, y1, r1, g1, b1, x2, y2, r2, g2, b2, output_path):
    ...
    row_padded = (image_width * 3 + 3) & (~3)  # Row padding to make the row size a multiple of 4 bytes
    ...
    for row in range(image_height):
        for col in range(image_width):
            # Calculate the interpolation factor (t) based on the horizontal position (col)
            t = (col - x1) / (x2 - x1) if x2 != x1 else 0  # Prevent division by zero
